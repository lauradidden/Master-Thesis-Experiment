...
class MostFrequentVariantExampleRetriever(ResultSetCharacterizer):
    ...
    def _retrieve_samples_from_log(self, log: pd.DataFrame, valid_items):
        ...
        if len(valid_items) > self.k:
            log = pm4py.filter_variants_top_k(log, self.k)
            valid_items = set(log[MostFrequentVariantExampleRetriever.case_id_glue].unique())

        random_samples = random.sample([*valid_items], 1)
        return log[log[MostFrequentVariantExampleRetriever.case_id_glue].isin(random_samples)]

    @staticmethod
    def _print_properties(header: str, df: pd.DataFrame):
        if df is not None:
            print(header)
            case_ids = set(df[MostFrequentVariantExampleRetriever.case_id_glue].unique())
            for case_id in case_ids:
                example = df[df[MostFrequentVariantExampleRetriever.case_id_glue] == case_id]
    ...
    def get_properties(self, result_set: pd.DataFrame, reference_log: pd.DataFrame):
        result_set_items = set(result_set[MostFrequentVariantExampleRetriever.case_id_glue].unique())
        reference_log_items = set(reference_log[MostFrequentVariantExampleRetriever.case_id_glue].unique())
        ...
        MostFrequentVariantExampleRetriever._print_properties(f'Sample from {result_set.name}', t1)
        MostFrequentVariantExampleRetriever._print_properties(f'Sample from {reference_log.name}', t2)
        MostFrequentVariantExampleRetriever._print_properties(f'Sample in {result_set.name} and {reference_log.name}', t3)
